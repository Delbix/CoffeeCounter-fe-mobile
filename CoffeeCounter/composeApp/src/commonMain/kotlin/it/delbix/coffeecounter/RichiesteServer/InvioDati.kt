...
object InvioDati: InvioDatiService {
     ...
     * @param notifica [Notifica] = struttura della notifica
     * @return HttpResponse, con il contenuto della risposta del server
     */
    override suspend fun inviaNotifica(notifica: Notifica): String {
        val response = client.post("${AppInit.API_URL}/angular/notification") {
            contentType(ContentType.Application.Json)
            setBody(Json.encodeToString(notifica))
        }
        return response.bodyAsText()
    }
}