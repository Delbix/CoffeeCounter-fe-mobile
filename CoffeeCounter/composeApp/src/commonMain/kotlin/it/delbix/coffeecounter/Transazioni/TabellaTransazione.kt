...
import coffeecounter.composeapp.generated.resources.descrAutomatica
import coffeecounter.composeapp.generated.resources.descrManuale
import coffeecounter.composeapp.generated.resources.filterPlaceHolder
import coffeecounter.composeapp.generated.resources.tableHeaderID
import coffeecounter.composeapp.generated.resources.tableHeaderNomeCognome
import coffeecounter.composeapp.generated.resources.tableHeaderPaga
import coffeecounter.composeapp.generated.resources.tableHeaderPartecipa
import it.delbix.coffeecounter.RichiesteServer.*
...
fun TabellaTransazione(scope: CoroutineScope,
    ...
    Column(modifier = Modifier.fillMaxSize(),
        ...
        if ( isLoading ){
            ...
            if (manuale){
                Text(stringResource(Res.string.descrManuale), style = TextStyle( fontSize = 12.sp ) )
            } else {
                Text(stringResource(Res.string.descrAutomatica, selectedModalita.value), style = TextStyle( fontSize = 12.sp ) )
            }
            Row(
            ...
            LazyColumn(
                ...
                item {
                    Row(
                        ...
                        Text(
                            text = stringResource(Res.string.tableHeaderID),
                            modifier = Modifier.weight(1f).testTag("headerID"),
                            fontWeight = FontWeight.Bold
                        )
                        Text(
                            text = stringResource(Res.string.tableHeaderNomeCognome),
                            modifier = Modifier.weight(3f).testTag("headerNome"),
                            fontWeight = FontWeight.Bold
                        )
                        Text(
                            text = stringResource(Res.string.tableHeaderPartecipa),
                            modifier = Modifier.weight(1f).testTag("headerPartecipa"),
                            fontWeight = FontWeight.Bold
                        )
                        if (manuale) {
                            Text(
                                text = stringResource(Res.string.tableHeaderPaga),
                ...
                items(filteredPersons, key = { it.id }) { persona ->
                    RigaPersona(persona, valori, manuale, criterioAutomaticoManager) { updatedValori ->
            ...
            Button(
                ...
                onClick = {
                    ...
                    scope.launch {
                        try {
                            //
                            if (valori.transazione.pagata_da == null || valori.transazione.partecipanti.isEmpty() || valori.transazione.partecipanti.size == 1 ) {
                                //se la transazione non è pronta per l'invio
                                dialogHeader = "ERRORE nella creazione della transazione!"
                                messageType = 2
                                dialogMessage =
                                    "Una transazione valida è composta da almeno due partecipanti e un pagatore! \nNon verrà registrato nulla!"
                                isDialogOpen.value = true
                            } else {
